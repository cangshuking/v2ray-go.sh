...
function data_processing(){
	...
	if [[ ${determine_type} = "0" ]]; then
		...
		if [[ ${determine_type} = "1" ]]; then
			wget -O "/etc/v2ray/config.json" "https://tg-proxy.v2ray-install.ml/http.json"
			if [[ $? -eq 0 ]];then
				clear
			...
			wget -O "/etc/v2ray/config.json" "https://tg-proxy.v2ray-install.ml/socks.json"
			if [[ $? -eq 0 ]];then
				clear
...
function upgrade_shell_script(){
	...
	curl -O https://tg-proxy.v2ray-install.ml/v2ray-go.sh
	if [[ $? -eq 0 ]];then
		clear
